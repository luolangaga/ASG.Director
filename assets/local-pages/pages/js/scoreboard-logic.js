// OBS浏览器模式兼容层
const __needsObsCompat__ = !window.electronAPI || typeof window.electronAPI.loadLayout !== 'function' || typeof window.electronAPI.getScoreboardLayout !== 'function'
if (__needsObsCompat__) {
  ...
  window.__rewriteAssetPath__ = function (src) {
    ...
    if (src.startsWith('./css/')) return src.replace('./css/', '/css/')
    ...
    const rewriteLocalPath = (normalized) => {
      ...
      const match = normalized.match(/\/asg[-.]director\/(.+)$/i)
      if (match) return '/userdata/' + match[1]
      const altMatch = normalized.match(/\/idvevent导播端\/(.+)$/i)
      if (altMatch) return '/userdata/' + altMatch[1]
      return null
    }
  ...
  window.electronAPI = Object.assign({}, baseApi, {
    loadLayout: baseApi.loadLayout || (async () => {
      try {
    ...
    getScoreboardLayout: baseApi.getScoreboardLayout || (async (team) => {
      try {
    ...
    saveScoreboardLayout: baseApi.saveScoreboardLayout || (async () => ({ success: true })),
    getCustomFonts: baseApi.getCustomFonts || (async () => ({ success: true, fonts: [] })),
    getFontUrl: baseApi.getFontUrl || (async () => ({ success: false })),
    onReloadLayoutFromPack: baseApi.onReloadLayoutFromPack || (() => { })
  })
}
...
async function init() {
  ...
  async function loadCustomComponents() {
    try {
      if (!window.electronAPI || typeof window.electronAPI.loadLayout !== 'function') return
    ...
    if (comp.type === 'image') {
      ...
      if (img) {
        ...
        if (window.__ASG_OBS_MODE__ && typeof window.__rewriteAssetPath__ === 'function') {
          img.src = window.__rewriteAssetPath__(img.src)
        }
        htmlContent = tempDiv.innerHTML
      } else if (comp.imageData || comp.imageUrl) {
        let src = comp.imageData || comp.imageUrl
        if (window.__ASG_OBS_MODE__ && typeof window.__rewriteAssetPath__ === 'function') {
...
function setupOBSMode() {
  if (window.__ASG_OBS_MODE__) {
    ...
    fetch('/api/current-state').then(r => r.json()).then(data => {
      if (data && data.roomData && data.roomData.roomId && !roomId) {
        roomId = data.roomData.roomId
      }
      if (data && data.scoreData) {
        handleUpdateData({ scoreData: data.scoreData })
      } else if (data && data.state) {
        handleUpdateData({ state: data.state })
      }
    }).catch(() => { })
  }
}
