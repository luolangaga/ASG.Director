...
class OBSWebSocket extends EventEmitter {
  ...
  /**
   * 切换媒体源输入（URL 或本地文件）
   * @param {string} sourceName - 媒体源名称
   * @param {string} mediaUrl - URL 或本地文件路径
   * @param {Object} options - 选项
   * @param {boolean} options.isLocalFile - 是否本地文件（默认自动判断）
   * @param {boolean} options.overlay - 是否 overlay（默认 true）
   * @param {boolean} options.restart - 切换后是否尝试重启媒体源（默认 true）
   * @returns {Promise<void>}
   */
  async switchMediaInput(sourceName, mediaUrl, options = {}) {
    const source = typeof sourceName === 'string' ? sourceName.trim() : ''
    const input = typeof mediaUrl === 'string' ? mediaUrl.trim() : ''
    if (!source) throw new Error('sourceName 不能为空')
    if (!input) throw new Error('mediaUrl 不能为空')
    ...
    if (isLocalFile) {
      inputSettings.local_file = input
    } else {
      inputSettings.local_file = ''
    }

    await this.setInputSettings(source, inputSettings, options.overlay !== false)
